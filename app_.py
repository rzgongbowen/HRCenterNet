...
def _nms(img, predict, out_path, filename, nms_score, iou_threshold):
    ...
    if len(bbox) > 0:
        ...
        for k in _nms_index:
            ...
            # if os.path.exists(str(out_path) + '\\cropped_out\\'):
            #     shutil.rmtree(str(out_path) + '\\cropped_out\\')  # 递归删除目标文件夹及其所有内容
...
def process_image(filepath):
    try:
        ...
        return temp_output_path, cropped_out_path  # 返回临时处理后的图像路径
...
def upload_image():
    ...
    if file:
        try:
            ...
            # 处理图像并返回处理后的图像路径
            result_filepath, cropped_out_path = process_image(filepath)

            print(f'上传的带预测的图片路径：{filepath}')
            print(f'预测出的带标注的图片路径：{result_filepath}')
            print(f'分割好的图片文件夹路径：{cropped_out_path}')

            return jsonify({'success': True, 'result': result_filepath})
        except Exception as e:
